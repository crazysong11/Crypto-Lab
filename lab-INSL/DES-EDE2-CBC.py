...
def DES(text, key, mode):
    ...
    for subkey in subkeylist:
        ...
        S_Input = int(Rn_expand, 2) ^ int(subkey, 2)
...
def p7p(text):
    ...
    return text + "{:02x}".format(8 - len(l) % 8) * (8 - len(l) % 8)
...
if __name__ == '__main__':
    ...
    # 转二进制
    k1 = bin(int(k1, 16))[2:].zfill(64)
    k2 = bin(int(k2, 16))[2:].zfill(64)
    ...
    if mode == 1:
        ...
        i = 0
        while i < len(s):
            mingwen = bin(int(s[i:i + 16], 16) ^ int(iv, 16))[2:].zfill(64)
            mingwen = DES(mingwen, k1, mode)
            mingwen = DES(mingwen, k2, not bool(mode))
            mingwen = DES(mingwen, k1, mode)
            iv = hex(int(mingwen, 2))
            ans = iv[2:]
            l = len(ans)
            while l < 16:
                ans = "0" + ans
        ...
        i = 0
        while i < len(s):
            mingwen = bin(int(s[i:i + 16], 16))[2:].zfill(64)
            copy = hex(int(mingwen, 2))

            mingwen = DES(mingwen, k1, mode)
            mingwen = DES(mingwen, k2, not bool(mode))
            mingwen = DES(mingwen, k1, mode)
            ans = hex(int(iv, 16) ^ int(mingwen, 2))[2:]
            l = len(ans)
            while l < 16:
                ans = "0" + ans
                l += 1
